...
### 캐시 무효화
- 캐시를 적용 안 해도 브라우저들이 임의로 캐시를 해버림
- 캐시가 되어서는 안 될 경우, 하단 헤더를 모두 넣어줘야 함
  - Cache-Control
    - no-cache: 데이터는 캐시해도 되지만, 항상 원 서버에 검증 사용
    - no-store: 데이터에 민감한 정보가 있으므로 저장하면 안됨
    - must-revalidate: 캐시 만료 후 최초 조회 시 원 서버에 검증
  - Pragma
    - no-cache (과거 브라우저일 경우)

- no-cache, must-revalidate
  - 프록시 서버에서 오리진 서버로 가는 네트워크에 오류가 발생했을 경우, 프록시 서버는 이전 데이터를 리턴할 수 있음
  - must-revalidate를 사용하면, 네트워크 오류로 원 서버에 접근할 수 없는 경우 504 에러를 리턴함
